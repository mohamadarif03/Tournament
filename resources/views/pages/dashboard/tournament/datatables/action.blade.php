<div class="flex">
    <a href="{{ route('tournament.edit', $data->id) }}" type="button"
        class="bg-yellow-400 hover:bg-yellow-600 text-white font-bold py-2 px-4 rounded mb-5 mr-6">
        <span>Edit</span>
    </a>
    <form onsubmit="return confirm('Yakin Ingin Menghapus Data Ini?')" method="POST" action="{{ route('tournament.destroy', $data) }}">
        @method('DELETE')
        @csrf
