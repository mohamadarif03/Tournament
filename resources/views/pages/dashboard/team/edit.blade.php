...
@section('content')
    <div class="grid grid-cols-1 lg:grid-cols-1 gap-7">
        <form enctype="multipart/form-data" action="{{ route('team.update', $team) }}" class="theme-form theme-form-2 mega-form" method="POST">
            @csrf
            @method('PUT')
            <div class="border border-black/10 dark:border-white/10 p-5 rounded-md">
                ...
                <div class="mb-4">
                    ...
                    <input
                        ...
                        placeholder="Nama" type="text" id="name" name="name" value="{{ $team->name }}" />
                </div>

                <div class="mb-4">
                    <label class="mt-1.5 font-semibold">Logo</label>
                    @if ($team->logo)
                        <img src="{{ asset('storage/' . $team->logo) }}" alt="logo" width="150" srcset="">
                    @endif
                    <input
                        class="form-input w-full rounded-lg border border-black/10 dark:border-white/10 bg-transparent px-3 py-2.5 placeholder:text-black/60"
                ...
                <div class="mb-4">
                    ...
                    <textarea id="description" name="description" rows="4"
                        ...
                        placeholder="Deskripsi"value="{{ $team->description }}">{{ $team->description }}</textarea>
                </div>
                <div class="mb-4">
                    ...
                    <select id="countries" name="game_id"
                        ...
                        @foreach ($games as $game)
                            <option value="{{ $game->id }}" <?php if ($team->game_id == $game->id) {
                                echo 'selected';
                            } ?>>{{ $game->name }}</option>
                        @endforeach
                    </select>
